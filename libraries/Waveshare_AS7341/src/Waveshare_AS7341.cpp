...
{
	DEV_I2C_Init(AS7341_ADDRESS);//8-bit address
    //Serial.print("ID = 0x%x\r\n",AS7341_Read_Byte(AS7341_ID));//get AS7341 id	
  while(DEV_I2C_Query() != 0)
  {
    Serial.print("Could not find AS7341,Try to find again\r\n");
    DEV_Delay_ms(4000);
  }
	AS7341_Enable(true);
	measureMode=mode;
...
info：	
******************************************************************************/
void AS7341_EnableSpectralMeasure(bool flag)//Enable spectral measurement
{
    UBYTE data;
    data=AS7341_Read_Byte(AS7341_ENABLE);
    if(flag == true){
...
info：	
******************************************************************************/
void AS7341_EnableSMUX(bool flag)//Enable multiplexer
...
info：	
******************************************************************************/
void AS7341_EnableFlickerDetection(bool flag)
{
...
UBYTE AS7341_ReadFlickerData()
{
	  ...
	  UBYTE retry = 100;
	  if(retry == 0) Serial.print(" data access error");
...
info：true		: Enable
			false		: Anergy
******************************************************************************/
void AS7341_EnableLED(bool flag)
{
  UBYTE data=0;
  UBYTE data1=0;
  AS7341_SetBank(1);
...
void AS7341_INTerrupt()
{
  ...
  if(data & 0x80){
    Serial.print("Spectral interrupt generation ！\r\n");
  } else {
    return ;
  }
}
